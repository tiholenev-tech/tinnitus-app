...
> Статус към 2026-06-06. **✅ ЗАВЪРШЕН, РЕГИСТРИРАН, MERGED и LIVE.**
> PR **#60** merged в `main` (squash `9458c3e`) → деплойнато (app-shell **1.0.117**).
> Клонът `claude/zen-edison-X2rb1` е история.
>
> ✅ **ПРЕВОДЪТ Е 100% ЗАВЪРШЕН.** 1545/1545 листа. `i18n_validate.py ro` = **0 проблема**.
> `check_claims.py ro` = **0 BLOCK** (MDR чисто). **Регистрацията Е НАПРАВЕНА** (виж долу).
>
> Останалото в този файл е запазено като **референтен конвейер/правила** за следващ език.
...
- **Готови ~775 уникални низа (~59%):** ui-обвивка/onboarding, библиотека+шумове,
  правни текстове (privacy/terms), дневник, SOS дишане, научна основа (THI/notched/RCT),
  гласова диктовка, заглавия на профилите + 5-те кратки профил-резюмета, **+ цялата
  `categoryInfo` секция** (тела на 5-те профила tonal_high/noise_low/stress_sensitive/
  somatic_fluctuating/adapted_mild + всичките режими sleep_deep, falling_asleep,
  relaxation, daily, anxiety + начало на meditation, с FAQ-тата и whenUsed/whenNotUsed).
- **Остават ~493 уникални низа:** остатъкът от `meditation` режима, плюс `faq`, `cbt`,
...
## Регистрация — ✅ НАПРАВЕНА (стъпките са референтни за следващ език)
> Всичко долу е изпълнено и merged в PR #60: picker 🇷🇴, `SUPPORTED=['bg','it','ro']`,
> version bump 1.0.116 → 1.0.117 (SW + app.html синхронни), `names.ro=Română` в bg/it/ro.
