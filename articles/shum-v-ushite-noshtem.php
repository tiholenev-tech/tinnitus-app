<?php
/**
 * AURALIS — статия (SEO/GEO страница). Шаблон за всички статии.
 * Server-side HTML, Schema.org (Article + FAQPage + reviewedBy Physician),
 * семантичен HTML5, BLUF + Q&A + числа (за Google featured snippet + AI цитат).
 * Ползва реалния app CSS (tokens.css + base.css) за консистентен вид.
 */
...
$UPDATED = '2026-06-04';
// Рецензент — РЕАЛЕН УНГ специалист (попълва се с име/титла/линк след съгласие):
$REVIEWER_NAME = 'предстои';            // напр. „д-р Иван Иванов, УНГ"
$REVIEWER_URL  = '';                    // линк към негова страница/профил (sameAs)
// Рецензент се показва (текст + Schema) САМО когато е реален. Докато няма —
// нищо не се твърди публично (честно пред читателя и Google).
...
<!-- Schema: Article + MedicalWebPage + автор/рецензент + FAQPage (за AI цитиране) -->
...
{"@context":"https://schema.org","@graph":[
 ...
 {"@type":"FAQPage","mainEntity":[
   ...
   {"@type":"Question","name":"Какво наистина помага при шум в ушите?","acceptedAnswer":{"@type":"Answer","text":"Звукова терапия (бял/розов/кафяв шум), подход с премахната честота (notched — до −24,6 точки THI на 6-ия месец), и работа с тревогата (CBT/ACT) — едни от най-доказаните. Добавките (магнезий, гинко) рядко помагат над плацебо."}},
...
<style>
.art{max-width:680px;margin:0 auto;padding:0 18px 60px;position:relative;z-index:1;}
.crumb{font-size:13px;color:var(--text-faint);padding:14px 0;}
.crumb a{color:var(--text-muted);}
.art h1{font-size:30px;line-height:1.18;font-weight:900;letter-spacing:-0.02em;color:var(--text);margin:6px 0 10px;}
.art .meta{font-size:13px;color:var(--text-faint);margin-bottom:18px;}
.art .bluf{font-size:18px;line-height:1.55;color:var(--text);background:var(--glass-bg-base,hsl(220 25% 8%/.5));border:1px solid var(--border-color);border-radius:16px;padding:18px;margin-bottom:8px;}
.art h2{font-size:22px;font-weight:800;letter-spacing:-0.01em;color:var(--text);margin:30px 0 10px;}
.art h3{font-size:17px;font-weight:800;color:var(--text);margin:20px 0 6px;}
.art p{font-size:16.5px;line-height:1.65;color:var(--text-muted);margin:0 0 14px;}
.art strong{color:var(--text);font-weight:800;}
.art .note{font-size:13.5px;color:var(--text-faint);border-left:2px solid var(--accent);padding:8px 0 8px 12px;margin:14px 0;}
.art .cta{display:inline-flex;align-items:center;justify-content:center;min-height:52px;padding:15px 26px;border-radius:100px;font-weight:800;font-size:16px;color:#0b0b16;text-decoration:none;background:linear-gradient(135deg,var(--accent),var(--accent-2));box-shadow:0 8px 24px hsl(255 80% 50%/.4);margin:8px 0;}
.art .cta-box{text-align:center;border:1px solid var(--border-color);border-radius:16px;padding:20px;margin:26px 0;background:var(--glass-bg-base,hsl(220 25% 8%/.4));}
.art ul{margin:0 0 14px 18px;} .art li{font-size:16px;line-height:1.6;color:var(--text-muted);margin:6px 0;}
.art .src{font-size:13.5px;color:var(--text-faint);} .art .src li{font-size:13.5px;}
.art .reviewer{font-size:13px;color:var(--text-faint);border-top:1px solid var(--border-color);margin-top:28px;padding-top:16px;}
.art a.inl{color:var(--accent);}
</style>
</head>
<body>
...
<article class="art">
  <nav class="crumb"><a href="/">Начало</a> · <a href="/articles/">Статии</a> · Шум в ушите нощем</nav>
  ...
  <p class="note">Образователен текст, не лекарски съвет. При внезапен, едностранен или пулсиращ шум — на УНГ.</p>
  ...
  <p>Днес изследванията сочат, че тинитусът се ражда <strong>в мозъка</strong>, не в ухото. Когато слухът отслабне дори леко, мозъкът получава по-малко сигнал и <strong>усилва</strong> оставащия — като усилвател, надут докрай, който започва да съска сам. Потискащият тонус отслабва и невроните работят по-синхронно; този „вътрешен съсък" се възприема като постоянен звук.</p>
  ...
  <p>Страхът „това няма да спре" сам по себе си усилва шума — затова работата върху отношението към него е сред <strong>най-доказаните</strong> подходи. CBT и ACT не махат звука, а го свалят от позицията на заплаха. Шумът остава, но престава да командва.</p>
  ...
  <div class="cta-box">
    <strong style="color:var(--text);font-size:17px;">Чуйте разликата за 30 секунди</strong>
    <p style="margin:8px 0 14px;">Направете безплатния тест, намерете тона си и чуйте облекчението — без регистрация.</p>
