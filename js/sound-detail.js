...
window.SoundDetail = (function () {
  ...
  function openPlayer() {
    ...
    if (sound.recommended_mix_ratio) {
      // ratio expected format "70/30" → layer1=70, layer2=30
      var parts = String(sound.recommended_mix_ratio).split('/');
      if (parts.length === 2) {
        var l1 = parseInt(parts[0], 10);
        var l2 = parseInt(parts[1], 10);
        if (!isNaN(l1)) try { localStorage.setItem('auralis_player_layer1_vol', String(l1)); } catch (e) {}
        if (!isNaN(l2)) try { localStorage.setItem('auralis_player_layer2_vol', String(l2)); } catch (e) {}
      }
    }
